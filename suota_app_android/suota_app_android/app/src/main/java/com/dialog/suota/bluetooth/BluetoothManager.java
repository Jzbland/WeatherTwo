...
//		int memType = this.getSpotaMemDev();
		int memType=318767104;
		Log.i(TAG, "setSpotaMemDev: 发送参数memType"+memType);
	...
	private int getMemParamsSPI() {
		Log.i(TAG, "getMemParamsSPI: 我的参数"+MISO_GPIO+"=="+MOSI_GPIO+"=="+CS_GPIO+"=="+SCK_GPIO+((MISO_GPIO << 24) | (MOSI_GPIO << 16) | (CS_GPIO << 8) | SCK_GPIO));
	...
	public void setSpotaGpioMap() {
		...
		switch (memoryType) {
			case Statics.MEMORY_TYPE_SPI:
				...
				Log.i("info","meninfodata1   "+memInfoData);
				valid = true;
				break;
			case Statics.MEMORY_TYPE_I2C:
				//memInfoData = this.getMemParamsI2C();
				Log.i("info","meninfodata2   "+memInfoData);
				valid = true;
				break;
		}
		if (valid) {
			...
			Log.i(TAG, "setSpotaGpioMap: 发送参数memInfoData"+memInfoData);
...
//		int blocksizeLE = (blocksize & 0xFF) << 8 | ((blocksize & 0xFF00) >> 8);
		...
		Log.i(TAG, "setPatchLength: 发送参数blocksize"+blocksize);
	...
	public float sendBlock() {
		...
		if (!lastBlockSent) {
			...
			if (lastChunk) {
				...
				Log.i(TAG, "sendBlock: 发送下一个block");
				// SPOTA
